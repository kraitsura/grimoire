 ...
 */
const extractHunks = (patch: any): Hunk[] => {
  return patch.hunks.map((hunk: any) => ({
 ...
 */
export const DiffServiceLive = Layer.effect(
  DiffService,
  Effect.gen(function* () {
    const computeDiff = (
      oldContent: string,
    ...
    const formatInline = (diff: DiffResult): Effect.Effect<string, never, never> =>
      Effect.gen(function* () {
        const lines: string[] = []
