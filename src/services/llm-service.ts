...
const getProviderNameFromModel = (model: string): string => {
  ...
  if (lowerModel.includes("llama") || lowerModel.includes("mistral") || lowerModel.includes("mixtral")) {
    return "ollama"
  }
...
const makeLLMService = Effect.gen(function* () {
  ...
  const getProviderForModel = (
    model: string
  ): Effect.Effect<LLMProvider, LLMError> =>
    Effect.gen(function* () {
      ...
      return yield* getProvider(providerName)
    })
...
export const makeLLMServiceLayer = (
